<template>
  ...
  <div class="createPost-container">
    ...
    <el-table :data="result.impactResult" border fit highlight-current-row style="width: 100%">

      <el-table-column width="60px" align="center" label="序号">
        <template slot-scope="scope"> {{ scope.$index+1 }}</template>
      </el-table-column>
      <el-table-column align="center" label="特征频率名称">
        <template slot-scope="scope">
          <span>{{result.gearboxInfo.ratioInfoList[scope.row.ratioPosition-1].description }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="特征频率阶次(Hz)">
        <template slot-scope="scope">
          <span>{{result.gearboxInfo.ratioInfoList[scope.row.ratioPosition-1].value }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="特征频率">
        <template slot-scope="scope">
          <span>{{result.gearboxInfo.ratioInfoList[scope.row.ratioPosition-1].value * result.accurate1xInHz }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="计算峰峰值">
        <template slot-scope="scope">
          <span v-if="scope.row.detail.hit">{{  scope.row.detail.scaledPeakToPeak  }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="计算峰值系数">
        <template slot-scope="scope">
          <span v-if="scope.row.detail.hit">{{  scope.row.detail.scaledCrest }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="通频值">
        <template slot-scope="scope">
          <span v-if="scope.row.detail.hit">{{  scope.row.detail.overall }}</span>
        </template>
      </el-table-column>
